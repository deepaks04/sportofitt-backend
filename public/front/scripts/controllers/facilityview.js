    ...
    .controller('FacilityViewCtrl', function ($stateParams, $state, searchService, toastr, $scope, bookingService) {
        ...
        vm.init = function () {
            searchService.getFacilityById(vm.facilityId).then(function (response) {
                ...
                //simpleMap(vm.facility.latitude, vm.facility.longitude, draggableMarker);
            }).catch(function (response) {
                toastr.error(response);
            })

            searchService.getFacilityAvailableSlotsById(vm.facilityId).then(function (response) {
        ...
        vm.filterAvailableSlot = function () {
            var day = vm.filter.selectedDate.getDay();
            vm.selectedDay = (day != 0) ? day : 7;
            vm.filteredSlots = vm.facilitySlots[vm.selectedDay][vm.filter.slot];
            vm.selectedSlot = vm.filteredSlots[0];
        }

        vm.today = function () {
            vm.filter.selectedDate = new Date();
        };
